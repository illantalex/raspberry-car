...
def load_model(model_path):
    ...
    interpreter = tflite.Interpreter(model_path=model_path, num_threads=8)
...
def display_result(result, frame, labels, writer):
...
if __name__ == "__main__":
    ...
    while True:
        ...
        display_result(top_result, frame, labels, w)
        key = cv2.waitKey(1)
        fps.update()
        if key == 27:  # esc
            break
