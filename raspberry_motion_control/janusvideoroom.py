...
ap.add_argument(
    "-i",
    "--input",
    help="A full path to input video device",
    required=True
)
ap.add_argument(
    "-H",
...
VIDEO_SRC = f"v4l2src device={args['input']}"
...
if DO_VP8:
    (encoder, payloader, rtp_encoding) = (
        "vp8enc target-bitrate=400000 overshoot=25 undershoot=100 deadline=33000 keyframe-max-dist=1",
...
""".format(
    VIDEO_SRC, encoder, payloader, rtp_encoding
)
...
class JanusGateway:
    ...
    async def connect(self):
        ...
        self.conn = await websockets.connect(
            self.server, subprotocols=["janus-protocol"], ssl=sslCon
        )
        transaction = transaction_id()
        await self.conn.send(
            json.dumps({"janus": "create", "transaction": transaction})
        )
    ...
    async def _recv_and_parse(self):
        ...
        if janus == "event":
            ...
            return WebrtcUp(sender=raw["sender"])
        elif janus == "media":
            return Media(
                sender=raw["sender"], receiving=raw["receiving"], kind=raw["type"]
            )
        elif janus == "slowlink":
            return SlowLink(
                sender=raw["sender"], uplink=raw["uplink"], lost=raw["lost"]
            )
        elif janus == "hangup":
            return HangUp(sender=raw["sender"], reason=raw["reason"])
        elif janus == "ack":
            return Ack(transaction=raw["transaction"])
        else:
            return raw
...
if __name__ == "__main__":
    ...
    if not check_plugins():
        sys.exit(1)
    c = WebRTCClient(args["label"], args["host"])
    loop = asyncio.get_event_loop()
    try:
